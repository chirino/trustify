use crate::ai::model::{ChatState, Conversation, ConversationSummary};
...
async fn conversation_crud(ctx: &TrustifyContext) -> anyhow::Result<()> {
    ...
    assert!(
        conversation_v1.state.messages.len() > 1,
    ...
    update1
        .state
        .add_human_message("Are there any related CVEs affecting it?".into());
    ...
    assert!(
        conversation_v2.state.messages.len() > update1.state.messages.len(),
        "assistant should add more messages"
    );
